...
function loadStatsPage()
{
	var table = document.getElementById("stats_table");//Retrieve our table element
			...
			{
			if(t[i].cells[2].innerHTML > t[i].cells[3].innerHTML)
			{
				row_counter++;
				 t[i].cells[4].innerHTML = "CU Boulder";
			}
			else
				{
					col_counter++;
					t[i].cells[4].innerHTML = t[i].cells[1].innerHTML;
				}
			}
			document.getElementById("Wins").innerHTML = row_counter;
			document.getElementById("Losses").innerHTML = col_counter;
}
...
function switchPlayers(playerNum)
{
	var player = document.getElementById("player_selector");
	   ...
	   {
	       var athlete = players[i].name;
	       insert = insert + "<a class = 'dropdown-item' href='#' onclick = 'switchPlayers("+ i +")'>" + athlete +"</a>"
	   }
	   player.innerHTML=insert;
}
